...
fun RoomSample(repository: Repository) {
    ...
    val cursos = ArrayList<CursoEntity>()
    scope.launch {
    ...
    //inicializacion de 3 cursos
    val cursosForDatabase = ArrayList<CursoEntity>()
    for (i in 100..103) {
        val cursoEntity = CursoEntity(i, name = "Curso " + i.toString())
        cursosForDatabase.add(cursoEntity)
    }

    scope.launch {
        repository.insertCursos(cursosForDatabase)
    }
